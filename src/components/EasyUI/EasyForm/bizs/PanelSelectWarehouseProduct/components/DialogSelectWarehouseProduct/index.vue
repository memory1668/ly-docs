            ...
            <div class="w-200">
              ...
              >
                ...
                >
                  ...
                  ><span>{{ subCategory.name }} </span><span v-if="subCategory.num_not_sync_mini > 0">({{ subCategory.num_not_sync_mini }})</span></el-menu-item>
                </el-submenu>
              ...
              >
                <div v-if="product.is_show" class="product-item">
                  <div class="flex">
                    ...
                    <div>
                      <el-tag v-if="product.is_sync_mini" size="mini" type="success">已上架</el-tag>
                      <div v-else class="pt-10">
                        <el-checkbox
                          v-model="checked[product.id]"
                          size="mini"
                          border
                          @change="checkSku($event, product)"
                        >选择</el-checkbox>
                      </div>
                    </div>
                  </div>
            ...
            <div v-for="(item, checkedIndex ) in checkedList" :key="'checked-' +checkedIndex" class="selected-item">
              ...
              <div class="flex">
                ...
                <div class="ml-15 flex-1">
                  ...
                  <div class="mt-10 fz-13">
                    <span class="mr-5 color-red">￥{{ item.product.price_range }}</span>
                  </div>
                </div>

                <div class="ml-15 pt-2 fz-18">
                  <i class="el-icon-circle-close icon-remove" @click="removeItem($event, 1, item.product.id, checkedIndex)" />
                </div>
              </div>
            </div>
...
export default {
  ...
  created() {
    ...
    this.groupedCategoriesBarProducts()
  },
  ...
  methods: {
     ...
     * 选中 SKU
     */
    checkSku(isChecked, product) {
      if (isChecked) { // 选中，添加到选中列表
        if (this.limit > 0 && this.checkedList.length >= this.limit) {
          this.$message.error('最多选择 ' + this.limit + '个商品')
          this.$set(this.checked, product.id, false)
          return
        }
        ...
        for (let i = 0; i < this.checkedList.length; i++) {
          if (this.checkedList[i].product.id === product.id) {
            targetIndex = i
            break
          }
        }
    ...
    confirm() {
      ...
      setTimeout(() => {
        this.checkedList.forEach(item => {
          this.$set(this.checked, item.product.id, false)
        }, 1000)
        this.checkedList = []
      }, 1000)
    },
     ...
     * 获取所有分类和商品信息
     */
    groupedCategoriesBarProducts() {
      apiWarehouseProduct.groupedCategoriesBarProducts().then(res => {
