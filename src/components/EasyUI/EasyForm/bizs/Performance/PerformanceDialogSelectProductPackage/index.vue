<template>
  <div
    :class="{'panel-active': list.length > 0, 'panel-category': dealCategory}"
    :style="{'margin-left': dealCategory ? marginLeft : 0}"
  >
          ...
          <div v-if="!dealCategory || !item.category || (item.category.id === categoryList[activeIndex].id)" :key="'checked-' +checkedIndex" class="selected-item">
            ...
            <div v-if="item.type === 1" class="flex">
              ...
              <div class="ml-15 flex-1">
                ...
                <!-- 判断是否显示商品规格 -->
                <div v-if="item.product.has_many_sku && canSelectSku" class="mt-5 fz-13">
    ...
    <dialog-select-product-package
      v-model="list"
      v-bind="$attrs"
      :visible.sync="dialogVisible"
      :show-config-num="showConfigNum"
      :can-select-sku="canSelectSku"
...
<script>
// import DialogSelectProductPackage from './components/DialogSelectProductPackage/index'
...
export default {
  ...
  // components: { DialogSelectProductPackage },
      ...
      val.forEach(item => {
        if (item.type === TYPE_PRODUCT) {
          ...
          if (!this.canSelectSku) {
            selectedProductSkuPackageMap[item.product_id] = item
          } else {
          ...
          if (category.type === TYPE_PRODUCT) {
            ...
            category.products.forEach(product => {
              if (this.canSelectSku) {
                ...
                if (selectedProductSkuPackageMap[key]) {
                  list.push({
                    ...selectedProductSkuPackageMap[key],
