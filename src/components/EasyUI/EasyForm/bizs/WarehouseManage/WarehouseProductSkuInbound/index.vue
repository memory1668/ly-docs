<template>
  <div>
      ...
      >
        ...
        <template slot-scope="scope">
          <div v-if="col.editable">
            <el-input v-if="col.prop !== 'product'" v-model="skusTableData[scope.$index][col.prop]" size="mini" :placeholder=" col.label" />
            <div v-else>
              <remote-select
                v-model="skusTableData[scope.$index][col.prop]"
                placeholder="选择商品"
                :clearable="true"
                :multiple="false"
                :options="{ dataField:'list',labelField: 'name',valueField: 'id' }"
                :request="{ url: '/warehouse-manage/products', method: 'GET', headers: {}, params: { }}"
    ...
    <!--多 SKU 商品才可以添加规格-->
    <div class="mt-10">
      <el-button icon="el-icon-plus" size="mini" @click="addSkuRow">添加商品</el-button>
    </div>
  </div>
</template>
  ...
  data() {
    return {
      ...
      /**
         ...
         */
      columnsTemplate: [
        { label: '商品', editable: true, batchEditable: false, prop: 'product', width: 180 },
        { label: '单位', editable: false, batchEditable: false, prop: 'num', width: 80 },
        { label: '数量', editable: true, batchEditable: true, prop: 'num' },
        { label: '进货价', editable: true, batchEditable: true, prop: 'purchase_price' },
        { label: '进货金额', editable: false, batchEditable: true, prop: 'purchase_amount' },
        { label: '售价', editable: false, batchEditable: true, prop: 'price' },
        { label: '零售金额', editable: false, batchEditable: true, prop: 'amount' }
      ],
      /**
         * 标题栏
         */
