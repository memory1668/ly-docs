    ...
    <el-table v-if="propsTableData.length > 0" :data="propsTableData" border stripe>
      <el-table-column label="规格名" width="130" align="center">
        <template slot-scope="scope">
          <el-input v-model="propsTableData[scope.$index].name" size="mini" placeholder="规格名称" @input="propInput($event, scope.$index)" />
        </template>
      </el-table-column>
      <el-table-column label="规格值" align="left">
        <template slot-scope="scope">
          <div
            ...
            class="prop-option"
          >
            <el-input v-model="propsTableData[scope.$index].options[i].name" size="mini" placeholder="规格值" @input="optionInput($event, scope.$index, i)">
              <el-button v-if="i > 0" slot="append" icon="el-icon-close" size="mini" @click="removeOption(scope.$index, i)" />
            </el-input>
          </div>
          <el-button icon="el-icon-plus" size="mini" @click="addOption(scope.$index)" />
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="80px" align="center">
        <template slot-scope="scope">
          <el-button type="danger" size="mini" plain @click="removeProp(scope.$index)">移除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="operation-prop">
      <el-button v-if="propsTableData.length < 3" size="mini" @click="addProp">添加规格名</el-button>
    </div>

    <!--图标配置-->
    <div v-if="showIconTable" class="skus-title">规格图标</div>
      ...
      >
        ...
        <template slot="header" slot-scope="scope">
          <span :id="scope.$index" style="display: block;">
            {{ columns[index].label }}
            <el-tooltip v-if="columns[index].editable" class="item" effect="dark" :content="'批量修改' + columns[index].label" placement="top-start">
...
const defaultSkusTableData = [ // 如果不配置规格，则默认一行
  {
    purchase_price: 0,
    price: 0,
    sku_code: '',
...
export default {
  ...
  data() {
    return {
       ...
       */
      columnsTemplate: [
        { label: '售价（元）', isEdit: false, editable: true, prop: 'price' },
        { label: '进货价（元）', isEdit: false, editable: true, prop: 'purchase_price' },
        // { label: '初始库存', isEdit: false, editable: true, prop: 'inventory' },
        { label: '规格编码', isEdit: false, editable: true, prop: 'sku_code' }
      ],
      /**
       * 标题栏
       */
       ...
       * SKU 表格值
       */
      skusTableData: JSON.parse(JSON.stringify(defaultSkusTableData)),
  ...
  methods: {
    ...
    initFormData() {
      ...
      this.formData = {
        props: [],
        skus: JSON.parse(JSON.stringify(defaultSkusTableData))
      }
      this.formData = { ...this.formData, ...this.value } // 添加配置的默认值
    },
    ...
    transFormDataToInnerData() {
      ...
      this.propsTableData.forEach((prop, propIndex) => {
        propCols.push({ label: prop.name, editable: false, prop: 'prop_' + propIndex })
    ...
    addProp() {
      this.__addProp()
      // if (this.propsTableData.length === 0) {
      //   this.__addProp()
      //   return
      // }
      // this.$confirm('添加规格名称将重置商品规格数据配置，确认操作？', '提示', { type: 'info' }).then(() => {
      //   this.__addProp()
      // }).catch(() => {})
    },
     ...
     */
    __addProp() {
      const lengthBefore = this.propsTableData.length
      this.propsTableData.push({ name: '', options: [{ name: '', icon: '' }], is_icon_open: false })
      // console.log('添加规格名称', this.columns)
      // 表头
      this.columns.splice(lengthBefore, 0, { label: '-', editable: false, prop: 'prop_' + lengthBefore })
     ...
     */
    addOption(index) {
      this.propsTableData[index].options.push({ name: '', icon: '' })
      this.addOptionDealOptionsIcon(index)
      this.calculateSkusTableData()

      // this.$confirm('添加规格值将重置商品规格数据配置，确认操作？', '提示', { type: 'info' }).then(() => {
      //   this.propsTableData[index].options.push({ name: '', icon: '' })
      //   this.addOptionDealOptionsIcon(index)
      //   this.calculateSkusTableData()
      // }).catch(() => {})
     ...
     */
    removeProp(index) {
      this.propsTableData.splice(index, 1)
      this.columns.splice(index, 1)

      this.calculateSkusTableData()
      this.removePropsOptionsIcon(index)

      // this.$confirm('移除规格配置将重置商品规格数据配置，确认操作？', '提示', { type: 'info' }).then(() => {
      //   this.propsTableData.splice(index, 1)
      //   this.columns.splice(index, 1)
      //
      //   this.calculateSkusTableData()
     ...
     * @param optionIndex
     */
    removeOption(propIndex, optionIndex) {
      this.propsTableData[propIndex].options.splice(optionIndex, 1)
      this.calculateSkusTableData()

      // this.$confirm('移除规格值将充值商品规格数据配置，确认操作？', '提示', { type: 'info' }).then(() => {
      //   this.propsTableData[propIndex].options.splice(optionIndex, 1)
      //   this.calculateSkusTableData()
      // }).catch(() => {})
    },
     ...
     */
    propInput(val, index) {
      this.columns[index] = { ...this.columns[index], label: val }
